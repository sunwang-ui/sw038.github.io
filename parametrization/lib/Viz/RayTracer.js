...
import Renderer from "/parametrization/lib/Viz/2DRenderer.js"
...
export default class RayTracer extends Renderer {
  ...
  async init() {
    ...
    // Retrieve the adapter limits
    const adapterLimits = adapter.limits;
    // Get a GPU device
    this._device = await adapter.requestDevice({
      requiredLimits: {
        maxBufferSize: Math.min(2 ** 30 - 1, adapterLimits.maxBufferSize), // max: 1GB
        maxStorageBufferBindingSize: Math.min(2 ** 30 - 1, adapterLimits.maxStorageBufferBindingSize), // max: 1GB
      }
    });
  ...
  resizeCanvas() {
    ...
    const tgtHeight = height * this._heightFactor; 
    ...
    this._bindGroup = this._device.createBindGroup({
      label: "Ray Tracer Renderer.js Bind Group",
